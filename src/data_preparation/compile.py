...
def prepare_assemblers(submissions: List[Submission]):
    def f(s: Submission):
        if not os.path.isfile("assembler/{}.s".format(s.submission_id)):
            ...
            assert res == 0
            res = subprocess.check_call(["docker", "cp", "atc:/{}.s".format(s.submission_id), "./assembler"])
            assert res == 0
...
def prepare_pps(submissions: List[Submission]):
    def f(s: Submission):
        if not os.path.isfile("pp/{}.cpp".format(s.submission_id)):
            res = subprocess.check_call("docker exec atc touch {}.cpp".format(s.submission_id), shell=True)
            assert res == 0
            command = "docker exec atc g++ -std=gnu++17 -w -O2 -DONLINE_JUDGE -I/opt/boost/gcc/include -L/opt/boost/gcc/lib -I/opt/ac-library -E -P /source_codes/{}.cpp > {}.cpp".format(
                s.submission_id, s.submission_id)
            res = subprocess.check_call(command, shell=True)
            assert res == 0
            res = subprocess.check_call(["docker", "cp", "atc:/{}.cpp".format(s.submission_id), "./pp"])
            assert res == 0
