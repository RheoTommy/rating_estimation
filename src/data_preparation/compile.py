...
def prepare_assemblers(submissions: List[Submission]):
    def f(s: Submission):
        ...
        res = subprocess.check_call(
            "docker cp cpp_compiler:/{}/Main.s ./assembler/{}.s".format(s.submission_id, s.submission_id), shell=True)
        assert res == 0
...
def prepare_pps(submissions: List[Submission]):
    def f(s: Submission):
        ...
        res = subprocess.check_call(command, shell=True)
        assert res == 0
        command = "docker exec cpp_compiler /bin/bash -c 'cd {} && g++-9 -std=gnu++17 -w -O2 -DONLINE_JUDGE -I/opt/boost/gcc/include -L/opt/boost/gcc/lib -I/opt/ac-library -P ./Main.cpp'".format(
            s.submission_id,
            s.submission_id)
        res = subprocess.check_call(command, shell=True)
        assert res == 0
        res = subprocess.check_call(
            "docker cp cpp_compiler:/{}/Main.cpp ./pp/{}.cpp".format(s.submission_id, s.submission_id), shell=True)
        assert res == 0
        res = subprocess.check_call("docker exec cpp_compiler rm -r {}".format(s.submission_id), shell=True)
        assert res == 0
